...
export default {
	...
	methods: {
		...
		changeOneUserData(index, rows){
			...
			_this.formTwo.id = id;
			_this.formTwo.name = rows[index].signature;
				...
				if(params.signature!=''){
					...
					allget(params, url).then(res => {
						...
						if(res.data.ret) {
							...
							_this.tabData.splice(_this.formTwo.index, 1, params); // 对tabData的数据中进行内容替换
						} else {
							baseConfig.errorTipMsg(_this, res.data.msg);
						}
					}).catch(function(error){
		...
		deleteOneUserData(index, rows) {
			...
			allget(params, url).then(res => {
				...
				if(res.data.ret) {
					...
					rows.splice(index, 1); // 从数据库里面删除成功，移除掉某一个需要移除的选项
					_this.totalpage--; //总条数进行减一
				} else {
					// 返回ret==0，非正常数据
					baseConfig.errorTipMsg(_this, res.data.msg);
				}
    ...
                }
            };
            axios.post(baseConfig.server+baseConfig.requestUrl+'/GlobalSet/addRandomSignature', formData, config)
            .then( (res) => {
				if(res.data.ret) {
					baseConfig.successTipMsg(_this, '增加成功'); // 添加成功的提示框提示
					setTimeout(function(){
						baseConfig.normalTipMsg(_this, '可以重新查询，加载最新数据！');
					}, 1000);
					// 提示让用户可以进行查询刷新数据更新
